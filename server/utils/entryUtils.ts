...
import type { CombinedEntryTemplate, EntryData } from "../../app/types/entry/entry";
import { EntryType, EntrySearchData, EntrySearchResults, EntrySearchParams } from "../../app/types/entry/entry";
import { entryTemplates } from "../../app/data/templates/pageTemplates";
import { v7 as uuidv7 } from "uuid";

const ENTRIES_DIRECTORY = path.join(process.cwd(), "data", "entries");
 ...
 * constructs the file path within the ENTRIES_DIRECTORY.
 * @param slug The slug of the entry. An empty string "" refers to "index.json"
 * at the root of ENTRIES_DIRECTORY.
...
export function getEntryFilePath(slug: string): string {
  ...
  return path.join(ENTRIES_DIRECTORY, `${fileName}.json`);
}
...
function migrateEntryData(
  ...
  if (rawEntryData.id) {
    ...
    // No ID present, generate a new uuidv7
    entryId = uuidv7();
    hasMigrated = true;
    console.warn(
      `Server: Generating new uuidv7 '${entryId}' for entry '${slug}' as no ID was present.`,
    );
  }
  ...
  const entryData: EntryData = {
    id: entryId, // Assign the (potentially new) uuidv7 here
...
async function getAllEntryData(): Promise<EntryData[]> {
  const entrySlugs = await readFilesRecursively(ENTRIES_DIRECTORY);
...
export async function getEntryById(entryId: string): Promise<EntryData | null> {
  const allEntries = await getAllEntryData();
...
export async function getEntryBySlug(slug: string): Promise<EntryData | null> {
  ...
  const allEntries = await getAllEntryData();
  const entry = allEntries.find((entry) => entry.slug === targetSlug);
...
export async function doesEntryExist(slug: string): Promise<boolean> {
  ...
  const allEntries = await getAllEntryData();
  return allEntries.some((entry) => entry.slug === targetSlug);
}
...
export async function fetchEntryStructure(): Promise<(EntryNode | FolderNode)[]> {
  ...
  // Step 1: Create all nodes (entries and folders) and populate the map
  ...
  const sortChildren = (nodes: (EntryNode | FolderNode)[]) => {
    nodes.sort((a, b) => {
      if (a.type === "folder" && b.type === "entry") return -1; // Folders before entries
...
/**
 * Searches for entries based on a query term across titles, categories, and tags.
 * Returns categorized lists of matching entries.
 * @param {EntrySearchParams} params - The search parameters.
 * @returns {Promise<EntrySearchResults>} An object containing categorized lists of matching entries.
 */
